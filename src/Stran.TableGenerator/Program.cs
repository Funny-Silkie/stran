            ...
            writer.BeginBlock("namespace Stran.Logics");
            {
                writer.BeginBlock("public partial class GeneticCodeTable");
                {
                    foreach ((string fileName, int index) in files)
                    {
                        ...
                        {
                            writer.BeginBlock("var dictionary = new Dictionary<Triplet, AA>(CompatibleSize)");
                            {
                            ...
                            writer.EndBlock(";");
                            writer.WriteLine($"var result = new {nameof(GeneticCodeTable)}(new ReadOnlyDictionary<Triplet, AA>(dictionary));");
                    ...
                    writer.BeginBlock($"public static partial {nameof(GeneticCodeTable)} GetNcbiTable(int id)");
                    {
                        ...
                        writer.WriteLine($"_ => throw new {nameof(ArgumentOutOfRangeException)}(nameof(id), $\"id'({{id}})'の遺伝暗号表は未定義です\"),");
                        writer.EndBlock(";");
                    }
                    writer.EndBlock();
                }
